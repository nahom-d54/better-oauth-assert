...
import { oAuthAssert } from ".";
import { oAuthAssertClient } from "./client";
...
describe("oauth2", async () => {
	...
	const testKey = await generateKeyPair("RS256");
	const clientAssertionFunction = async () => {
		...
		const jwt = await new SignJWT({})
			...
			.setIssuer(clientId)
			.setSubject(clientId)
			.setAudience("http://localhost:8081/token")
			.setIssuedAt(now)
