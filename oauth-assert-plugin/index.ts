...
/**
 * Configuration interface for generic OAuthAssert providers.
 */
export interface OAuthAssertConfig {
	...
	clientId: string;
	/**
	 * OAuth assertion type.
	 * This is used for client authentication in the token request.
	 * It should be in the format `urn:ietf:params:oauth:client-assertion-type:${string}`.
	 * @default "urn:ietf:params:oauth:client-assertion-type:jwt-bearer"
	 * @see {@link https://datatracker.ietf.org/doc/html/rfc7521}
	 */
...
interface OAuthAssertOptions {
	/**
	 * Array of OAuth provider configurations.
	 */
	config: OAuthAssertConfig[];
}
 ...
 */
export const oAuthAssert = (options: OAuthAssertOptions) => {
	const ERROR_CODES = {
			...
			const genericProviders = options.config.map((c) => {
				...
				return {
					...
					async validateAuthorizationCode(data) {
						...
						const clientAssertion = await c.clientAssertion();
						...
						return validateAuthorizationCode({
							...
							options: {
								...
								clientAssertionType:
									c.client_assertion_type ?? defaultAssertionType,
							},
			...
			oAuth2Callback: createAuthEndpoint(
				...
				async (ctx) => {
					...
					try {
						...
						const clientAssertion = await provider.clientAssertion();
						const defaultAssertionType =
							"urn:ietf:params:oauth:client-assertion-type:jwt-bearer";
						...
						tokens = await validateAuthorizationCode({
							...
							options: {
								...
								clientAssertion: clientAssertion,
								clientAssertionType:
									provider.client_assertion_type ?? defaultAssertionType,
							},
							tokenEndpoint: finalTokenUrl,
